...
const resaveImageFromStorage = async(imageUrl)=>{
  try {
    ...
    const imageSnapshot = await ImagesSaved.where("imageUrl", "==", imageUrl).get();
...
const deleteImageFromStorage = async (imageUrl) => {
  try {
    // Extraer el path del archivo de la URL
        ...
        // filePathMatch será un array donde:
        // filePathMatch[0] = "images/1729187488915-autoUnion2.webp"
        // filePathMatch[1] = "1729187488915-autoUnion2.webp"
...
const deleteImage = async(data, isId) => {
  try {
    ...
    if (isId) {
      ...
      // Búsqueda por URL
      const querySnapshot = await ImagesSaved.where("imageUrl", "==", data).limit(1).get();
